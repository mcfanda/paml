...
get_sample<-function(info) {
   ...
   ## gather info for clusters
    ...
    n<-dep$n
    if (n<4) {
      newn<-length(clustervalues$id)*n
      wdata<-.make_clusterdata("within",info,newn)
     .names<-names(wdata)
      wdata[["id"]]<-rep(clustervalues$id,each=n)

    } else {
    for (i in unique(clustervalues$id)) {
      onedata<-.make_clusterdata("within",info,n)
      onedata[["id"]]<-i
      wdata[[i]]<-onedata
    }
...
.make_clusterdata<-function(cluster,info,n,centerby=NULL) {
  ...
  if (is.something(factors)) {
    ...
    if (info$factors=="contrasts") data<-dummies
